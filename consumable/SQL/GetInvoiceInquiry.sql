...
SET @@sqlstate = @@sqlstate + '
	...
	LEFT JOIN (
		...
		''
			select  v1.TAX_INVOICE_NO AS TAX_INVOICE_NO, V1.TAX_INVOICE_DT AS TAX_INVOICE_DT from TMMIN_E_FAKTUR.dbo.tb_r_vat_in_h v1 
			WHERE 
			v1.REPLACEMENT_FG = (SELECT MAX(v2.REPLACEMENT_FG) FROM TMMIN_E_FAKTUR.dbo.TB_R_VAT_IN_H V2 
