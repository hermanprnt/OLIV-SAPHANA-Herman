...
SET @@sqlstate = @@sqlstate + '
	...
	LEFT JOIN (SELECT DISTINCT DD.SAP_VENDOR_ID
		...
		--,GRI.PO_DATE
		,CASE 
			WHEN GRI.PO_DATE <= DD.VALID_DD_TO THEN ''YES''
			ELSE ''NO''
		END AS DUE_DILLIGENCE
		,CASE 
			WHEN GRI.PO_DATE <= AN.EXP_DATE THEN ''YES''
			ELSE ''NO''
		END AS AGREEMENT_NO
			FROM TB_M_DUE_DILLIGENCE DD
			JOIN TB_M_AGREEMENT_NO AN ON AN.VENDOR_CODE = DD.VENDOR_CODE
			...
			JOIN TB_R_GR_IR_FROM_SAP GRI ON GRI.VEND_CODE = DD.SAP_VENDOR_ID ) DD ON DD.SAP_VENDOR_ID = INV.SUPPLIER_CD
