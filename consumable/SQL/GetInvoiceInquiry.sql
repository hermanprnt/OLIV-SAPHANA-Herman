...
SET @@sqlstate = @@sqlstate + '
	...
	LEFT JOIN (
		...
		''SELECT POH.CREATED_DT AS CREATED_DT
				,DD.DD_STATUS AS DD_STATUS 
				,AN.EXP_DATE AS EXP_DATE 	
				,DD.VALID_DD_TO AS VALID_DD_TO 
				,DD.SAP_VENDOR_ID AS SAP_VENDOR_ID
				,CASE 
					WHEN POH.CREATED_DT <= DD.VALID_DD_TO THEN ''''YES''''
					ELSE ''''NO''''
				END AS DUE_DILLIGENCE
				,CASE 
					WHEN POH.CREATED_DT <= AN.EXP_DATE THEN ''''YES''''
					ELSE ''''NO''''
				END AS AGREEMENT_NO
			FROM PAS_DB_QA.dbo.TB_M_DUE_DILLIGENCE DD 
			LEFT JOIN PAS_DB_QA.dbo.TB_R_PO_H POH ON DD.VENDOR_CODE = POH.VENDOR_CD
			LEFT JOIN PAS_DB_QA.dbo.TB_M_AGREEMENT_NO AN ON AN.VENDOR_CODE = DD.VENDOR_CODE
		'')
	) PAS
